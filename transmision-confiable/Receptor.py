...
class Receptor:
	...
	def recibir():#Capa de comunicacion. 
		while bandera: 
			...
			int.from_bytes(datosBytes, byteorder='big')
			if rv == datosBytes:#Si es igual al numero de paquete que estamos esperando. 
				...
				while paquete != -1 #Mientras el elemento que se quiera este en la lista.
					...
					rv++#Establecemos el nuevo inicio de la lista. 
				...
				int.from_bytes(datosBytes, byteorder='big')
				if rv + 10 > datosBytes and datosBytes > rv#Si esta dentro de la ventana, lo guardamos. 
					ventana.insertar(paqueteRecibido, datosBytes)
					
	def almacenar(datos):#Capa superior
		imagen = open("laImagen.jpg", "bw")#Abrimos archivo donde vamos a guardar la imagen. laImagen.bin
